...
Fixpoint count (v : nat) (s : bag) : nat:=
  match s with
    | nil => O
    | v'::t => if( v' =? v ) then (S (count v t)) else (count v t)
  end.  
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool:=
  match s1 with
    | nil => true
    | h1::t => if (leb 1 (count h1 s2)) then (subset t (remove_all h1 s2)) else false
  end.
...
Theorem eqb_reflexivity : forall (v:nat),
  v =? v =true.
Proof. intros. induction v as [|v' IHv]. 
  -simpl. reflexivity.
...
(count v s) + 1 = count v (add v s).
Proof.
intros v s.
induction s as [|h t].
  - simpl. rewrite eqb_reflexivity. reflexivity.
  - rewrite plus_comm.  simpl. rewrite eqb_reflexivity. reflexivity. 
Qed.
...
Proof.
  ...
  - (* l = cons *)
    ...
    simpl. rewrite -> IHl'. rewrite plus_comm. simpl.
    reflexivity.
Qed.
...
(* None because there is no base type like O in case of nat 
    and true/false in case of bool. 
    Since each construction is recursively dependent on baz, 
    there is no element in baz.
 *)
