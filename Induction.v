...
Theorem plus_O_n_firsttry' : forall n:nat,
  n = 0 + n.
...
induction n as [|n' IH].
 - simpl. reflexivity.
  -simpl. intros m. rewrite IH. reflexivity.
Qed.
...
Proof.
  ...
  -simpl. intros m. rewrite IH1. induction m as [|m' IHm].
  --simpl. reflexivity.
...
(* 
Destruct:
We iterate over all values of the variable and prove for every value.

Induction:
We start with a base case and prove that then we assume it to be true for some arbitrary n.
Now we prove for n+1. 

Destruct may fail in case of huge space of inputs. *)
*)
...
This follows directly from the definition of +.
    m=m
Next, suppose n = S n', where
We've to prove S n' + m = m + S n' with induction hypotheis being n' + m = m + n'
S (n' + m) = m + S n'
using Induction hyp
S (m + n') = m + S n'
using induction on m:
case 1: S (0 + n') = 0 + S n' which follows from definitio of +
case 2: S (S m' + n') = S m' + S n' ; IH2 = S (m' + n') = m' + S n'
S (S (m' + n')) = S (m' + S n')
using IH2
S (m' + S n') = S (m' + S n')
wgere LHS and RHHS are equal.
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal) 
  ...
  | S n' => match m with
    ...
    Proof: base case n=O:
        ...
        [true = O =? O]
        which is true by definition of eqb

      IH: [true = n' =? n']
      Induction case:
        [true = S n' =? S n'] ----(1)
      we end up in the second match n of fixpoint definition of eqb and we match Sn' 
      which leads up to 
      eqb n' n' which is in IH hence (1)
